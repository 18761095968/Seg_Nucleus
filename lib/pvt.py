...
class CFM(nn.Module):
    def __init__(self, channel):
        ...
        # 输入通道数为3*channel,输出通道数为channel,卷积核大小为3*3
...
class SAM(nn.Module):
    # num_in代表输入特征图的通道数，plane_mid是中间特征图的通道数，mids代表GCN模块中间节点数的平方根
    ...
    def forward(self, x, edge):
        ...
        n, c, h, w = x.size()
        # 该函数的输入参数是一个四维张量edge。
        # dim=1表示在第一个维度上进行softmax计算，这通常是将输出向量转换为概率分布的一种方式。
        # [:, 1, :, :]表示选取所有batch、第二个通道、所有行和列的元素，即选取概率分布中第二个类别的概率值。
        # 最后，unsqueeze(1)在第二个维度上添加了一个新的维度，使其变成一个三维张量，其维度为（batch_size, 1, height, width）。
...
class PolypPVT(nn.Module):
    def __init__(self, channel=32):
        ...
        path = 'C:/Users/lenovo/PycharmProjects/Polyp-PVT-main/pretrained_pth/pvt_v2_b2.pth'
        ...
        # 输入通道数是channel,输出通道数是1，表示只有一个二值化的掩码图，其中每个像素点的值为0或者1，可以用来表示每个像素点属于前景还是背景
        # 卷积核的大小也是1
    ...
    def forward(self, x):
        ...
        # 上采样，将两个特征图的尺寸变成原来的8倍，准确的说，prediction1和prediction2是原始图的1/8，这里会变成原始图尺寸
