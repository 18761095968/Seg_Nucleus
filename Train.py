...
def test(model, path):
    ...
    gt_root = '{}/labels/'.format(data_path)
    model.eval()
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    # test1path = '/root/autodl-tmp/TransUnet_CYL/data/Cell/val'
    if (epoch + 1) % 1 == 0:
        dataset_dice = test(model, test_path)
        logging.info('epoch: {}, dice: {}'.format(epoch, dataset_dice))
        print(dataset_dice)
        dict_plot['cell'].append(dataset_dice)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='train_log_1.log',
